...
func main() {
	...
	err = registerScopedTool(server, "bind-projects-to-catalog", "Bind one or more projects to a catalog so they can install apps from it. Projects must be Ready and have a platform kubeconfig; run preflight-project or wait-for-project first.", func(ctx context.Context, args BindProjectsToCatalogArgs) (*mcp_golang.ToolResponse, error) {
